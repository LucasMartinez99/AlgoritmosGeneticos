...
def evaluate(individual, crime_data, comisarias_data, incident_comisaria_map, max_time=15, mes_seleccionado=None, periodo_seleccionado=None):
    # Aplicar filtros
    ...
    total_patrullas_asignadas = 0  # Variable para controlar el total de patrulleras asignadas

    # Calcular la cantidad máxima de incidentes que puede manejar cada patrullera (introduciendo variabilidad)
    incidentes_por_patrullera = max(1, len(crime_data) // 39 + random.randint(-2, 2))
    ...
    for idx, incident in crime_data.iterrows():
        ...
        if incident_id in incident_comisaria_map:
            ...
            if response_time <= max_time * 60:
                ...
                if incidentes_manejados_por_comisaria[nearest_comisaria_idx - 1] < max_incidentes_manejables:
                    ...
                    if total_patrullas_asignadas < 39:
                        ...
                        incidentes_manejados_por_comisaria[nearest_comisaria_idx - 1] += 1
                        # Penalizar significativamente cuando excede sin agregar adecuadamente
                        ...
                        total_penalty += 5000  # Puedes ajustar esta penalización si lo consideras necesario
    ...
    # Asegurarse de que el total de patrulleras sea 39 exactas
    if total_patrullas_asignadas < 39:
        # Si el número de patrulleras asignadas es menor que 39, añadir el restante de manera uniforme
        ...
        # Si el número de patrulleras excede 39, reducir patrulleras aleatoriamente
        excess_patrullas = total_patrullas_asignadas - 39
        for i in range(excess_patrullas):
            idx = random.choice([i for i, val in enumerate(patrulleras_asignadas) if val > 1])
            patrulleras_asignadas[idx] -= 1
    ...
    for idx, incidentes in enumerate(incidentes_manejados_por_comisaria):
        if incidentes == patrulleras_asignadas[idx] * incidentes_por_patrullera:
            total_penalty -= 500 + 100 * incidentes  # Recompensa proporcional
    ...
    # Penalización por patrulleras sin incidentes asignados
    for idx, num_patrullas in enumerate(patrulleras_asignadas):
        if incidentes_manejados_por_comisaria[idx] == 0 and num_patrullas > 0:
            total_penalty += 3900  # Penalización por patrulleras sin uso

    # Calcular el valor de fitness
    fitness_value = total_response_time + total_penalty

    # Añadir un ruido aleatorio para incentivar la exploración
    fitness_value += random.uniform(-100, 100)
